...
def write_daily_bar(bar,bar_sec=5,last_close_px=None) :
    ...
    Note that format it into the fixed time bars will be done by the dailybar
    """
    import pandas as pd
    ...
    ti=TradingDayIterator(day_start, adj_start=False) # day maybe a sunday
    ...
    while day < day_end:  # day is the prevous night of the trading day
        ti.next()
        day1=ti.yyyymmdd()
        utc_e = int(TradingDayIterator.local_ymd_to_utc(day1, 18,0,0))
        ...
        j=np.searchsorted(bar[1:, 0], float(utc_e)-1e-6)
        ...
        print 'getting bar ', day+'-18:00', day1+'-18:00', ' , got ', j-i, 'bars'
    ...
    return barr, trd_day_start, trd_day_end
